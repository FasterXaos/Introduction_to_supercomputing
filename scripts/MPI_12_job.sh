...
tmpDir="${TMPDIR:-/tmp}"
tmpOutputFile="$tmpDir/mpi12_output_${SLURM_JOB_ID:-$$}.txt"
...
matchedLines="$(grep -a '^MPI_12,' "$tmpOutputFile" | tr -d '\r' || true)"

if [[ -z "$matchedLines" ]]; then
    echo "No lines starting with 'MPI_12,' found; nothing to append to CSV." >&2
    exit 2
fi
