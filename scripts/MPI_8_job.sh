...
mkdir -p "$RESULTS_DIR"
csvPath="$RESULTS_DIR/MPI_8.csv"

tmpDir="${TMPDIR:-/tmp}"
tmpOutputFile="$tmpDir/mpi8_output_${SLURM_JOB_ID:-$$}.txt"

srun -n "${SLURM_NTASKS:-2}" "$EXE_PATH" "$MESSAGE_SIZE" "$MODE" "$NUM_ITERATIONS" > "$tmpOutputFile" 2>&1 || jobExit=$?
jobExit=${jobExit:-0}
