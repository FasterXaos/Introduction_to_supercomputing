...
mkdir -p "$RESULTS_DIR"
csvPath="$RESULTS_DIR/MPI_7.csv"

tmpDir="${TMPDIR:-/tmp}"
tmpOutputFile="$tmpDir/mpi7_output_${SLURM_JOB_ID:-$$}.txt"

srun -n "${SLURM_NTASKS:-1}" "$EXE_PATH" "$MESSAGE_SIZE" "$NUM_ITERATIONS" "$COMPUTE_UNITS" "$MODE" "$SEED" > "$tmpOutputFile" 2>&1 || jobExit=$?
jobExit=${jobExit:-0}
