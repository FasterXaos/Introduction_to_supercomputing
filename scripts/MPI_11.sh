...
exeName="MPI_11"
jobScript="$scriptDir/MPI_11_job.sh"
csvPath="$resultsDir/MPI_11.csv"

processList=(4 6 8 9 16 32 64 128)
...
echo "Compiling $srcDir/MPI_11.cpp -> $binDir/$exeName"
if [[ ! -f "$srcDir/MPI_11.cpp" ]]; then
    echo "Source not found: $srcDir/MPI_11.cpp" >&2
    exit 1
fi

mpicxx -O3 -std=c++17 -march=native -o "$binDir/$exeName" "$srcDir/MPI_11.cpp"
...
printf '%s\n' "testType,gridRows,gridCols,numProcesses,commType,medianTimeSeconds,globalSum,runIndex,mpiEnv" > "$csvPath"
...
for numProcs in "${processList[@]}"; do
    ...
    for numIterations in "${numIterationsList[@]}"; do
        for (( runIndex=1; runIndex<=numRuns; runIndex++ )); do
            seed=$RANDOM
            sbatch --ntasks="$numProcs" \
                   --output="$logDir/MPI_11-%j.out" \
                   --error="$logDir/MPI_11-%j.err" \
            ...
            echo "$(date -Is) queued: procs=$numProcs grid=${gridRows}x${gridCols} iters=$numIterations run=$runIndex"
            # sleep 0.05
        done
    done
done
