...
int main(int argc, char** argv) {
    ...
    long long problemSize = std::stoll(argv[1]);
    ...
    if (processRank == 0) {
        fullA.resize(static_cast<size_t>(problemSize));
        fullB.resize(static_cast<size_t>(problemSize));

        std::mt19937_64 generator(static_cast<unsigned long long>(seed));
        std::uniform_real_distribution<double> distribution(0.0, 1.0);

        for (long long i = 0; i < problemSize; ++i) {
            fullA[static_cast<size_t>(i)] = distribution(generator);
            fullB[static_cast<size_t>(i)] = distribution(generator);
        }

        long long base = problemSize / numProcesses;
        int remainder = static_cast<int>(problemSize % numProcesses);
        int offset = 0;
    ...
    int localCount = sendCounts[processRank];
    std::vector<double> localA(static_cast<size_t>(localCount));
    std::vector<double> localB(static_cast<size_t>(localCount));
    ...
    for (int i = 0; i < localCount; ++i) {
        localDot += localA[static_cast<size_t>(i)] * localB[static_cast<size_t>(i)];
    }
